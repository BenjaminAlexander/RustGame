...
impl RollingStats {
    ...
    pub fn add_value(&mut self, value: f64) -> Option<ValueOfInterest> {
        ...
        if self.roll_std_dev.is_full() {
            ...
            if (rolling_average - value).abs()
                > rolling_standard_deviation * self.standard_deviation_ration
            {
            ...
            if (average - rolling_average).abs()
                > standard_deviation * self.standard_deviation_ration
            {
            ...
            let rolling_average_min_max_change =
                match self.roll_av_min_max.add_value(rolling_average) {
                    None => None,
