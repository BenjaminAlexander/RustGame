...
impl TimeValue {
    ...
    const fn from_signed_nanos(seconds_since_epoch: u64, nanos: i32) -> Self {
        let mut normalized_seconds =
            (seconds_since_epoch as i64 + (nanos as i64 / TimeValue::NANOS_PER_SEC as i64)) as u64;
    ...
    pub fn from_secs_f64(value: f64) -> Self {
        ...
        let nanos =
            ((value - seconds_since_epoch as f64) * TimeValue::NANOS_PER_SEC as f64).round() as u32;
    ...
    pub fn as_secs_f64(&self) -> f64 {
        return (self.seconds_since_epoch as f64)
            + (self.nanos as f64) / (TimeValue::NANOS_PER_SEC as f64);
    }
...
mod tests {
    ...
    fn timer_value_test() {
        ...
        let system_time_duration_since_epoch = time_value
            .to_system_time()
        ...
        let time_value = TimeValue::from_system_time(
            &SystemTime::UNIX_EPOCH.add(Duration::new(23, 750_000_000)),
        )
        .unwrap();
