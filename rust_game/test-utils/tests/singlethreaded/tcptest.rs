...
#[test]
fn test_tcp() {
    LoggingConfigBuilder::new().add_console_appender().init(LevelFilter::Info);
    ...
    let connection_handler_sender = server_factory.new_thread_builder()
        .name("TcpConnectionListener")
        .spawn_tcp_listener(listen_socket.clone(), connection_handler, AsyncJoin::log_async_join)
        .unwrap();
    ...
    let client_thread_builder = client_factory.new_thread_builder().build_channel_thread::<EventOrStopThread<()>>();
...
fn test_write(factory: &SingleThreadedFactory, write_connection: &Arc<Mutex<Option<TestConnection>>>, read_connection: &Arc<Mutex<Option<TestConnection>>>, value: u32) {
    ...
    assert_eq!(value, read_connection.lock().unwrap().as_ref().unwrap().last_value.unwrap());
}
...
impl TcpConnectionHandlerTrait for ConnectionHandler {
    ...
    fn on_connection(&mut self, tcp_sender: ChannelTcpWriter, tcp_receiver: SingleThreadedReceiver<Vec<u8>>) -> ControlFlow<()> {
        ...
        let reader_sender = self.factory.new_thread_builder()
