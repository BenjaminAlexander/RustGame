...
pub fn main() {
    ...
    buf.push(4);
    buf.push(3);
    let mut fragments = MessageFragment::make_fragments(1, buf, 15);
    ...
    let mut a = FragmentAssembler::new(3);

    a.add_fragment(fragments.pop().unwrap());
    a.add_fragment(fragments.pop().unwrap());
    let fm = a.add_fragment(fragments.pop().unwrap()).unwrap();
